...
public class Operator {
	...
	public static ResultSet getListDetilTransaksi(Connection con,
			int idTransaksi) {
		return select(con, "SELECT * FROM detiltrans WHERE notrans="
				+ idTransaksi);
	}
	...
	public static boolean tambahBarang(Barang brg, Connection con) {
		return update(
				con,
				"INSERT INTO products " + "VALUES('" +brg.getIdproducts() +"','" +brg.getNamaproducts() +"','"
				+brg.getIdsupplier() + "'," +brg.getHarga() +"," +brg.getStok() +")");
	}
	...
	public static boolean tambahTransaksi(Transaksi trns, Connection con) {
		...
		for (int i = 0; i < trns.getDetilTransaksi().size(); i++) {
			returnValue = returnValue
					& update(con, "INSERT INTO detiltrans "
							+ "VALUES("
							+ id
							+ ", '"
	...
	public static boolean hapusTransaksi(Transaksi trns, Connection con) {
		...
		if (trns == null) {
			...
			returnData = returnData
					& update(con,
							"DELETE FROM detiltrans WHERE notrans="
									+ trns.getId());
			returnData = returnData
	...
	public static User checkLogin(User usr, Connection con) {
		ResultSet rs = select(con, "SELECT admin FROM pengguna "
				+ "WHERE username='" + usr.getUsername() + "' AND pass ='"
				+ usr.getPass() + "'");
		try {
			if (rs.next()) {
				return new User(usr.getUsername(), usr.getPass(),
						rs.getBoolean("admin"));
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
